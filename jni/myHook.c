...
uint32_t dvmLoadNativeCode_replace(const char* a1, uint32_t a2, uint32_t a3)
{
    ...
    if(strstr(a1, target_package) && xx_isDone)
    {
        ...
        if(handler)
        {
            ...
            pid_t pid = getpid();
            LOGI("pid is %d", pid);
            uint32_t module_base = get_module_base(pid, "/data/app-lib/com.bilibili.azurlane-1/libtolua.so");
            if(module_base)
            {
                LOGI("module_base:%x", module_base);
                uint32_t lua_loadx_target = module_base + 0x3BCB4;
                ZzHook((void *)lua_loadx_target, (void *)&lua_loadx_replace, (void **)&lua_loadx_ori, NULL, NULL, false);
            }
...
void so_entry(char *p)
{
    ...
    if(module_base)
    {
        ...
        ZzHook((void *)dvmLoadNativeCode_target, (void *)&dvmLoadNativeCode_replace, (void **)&dvmLoadNativeCode_ori, NULL, NULL, false);
        LOGI("hook done!");
    }    
